...
int pin_user_buffer(uint64_t uaddr,
                    size_t size,
                    user_addr_t *buf)
{
    unsigned long pagestart = ALIGN_DOWN(uaddr, PAGE_SIZE);
    unsigned long offset = offset_in_page(uaddr);
    unsigned long npages = DIV_ROUND_UP(offset + size, PAGE_SIZE);
    ...
    long pinned = pin_user_pages(pagestart,
                                npages,
    ...
    void *base_kaddr = vmap(pages, npages, VM_MAP, PAGE_KERNEL);
    if (!base_kaddr) {
    ...
    buf->pages = pages;
    buf->npages = npages;
    buf->base_kaddr = base_kaddr;
    buf->kaddr = (char *)base_kaddr + offset;

    return 0;
}

void unpin_user_buffer(user_addr_t *buf)
{
    if (!buf)
        return;

    if (buf->base_kaddr)
        vunmap(buf->base_kaddr);
...
int match_vector_op(struct ggml_compute_params *kparams, struct ggml_tensor *kdst)
{
    switch (kdst->op) 
    {
        case GGML_OP_RMS_NORM:
        {
            ggml_compute_forward_rms_norm_f32(kparams, kdst);
            
            printk(KERN_DEBUG "[Kernel] Matched GGML_OP_RMS_NORM.\n");  
        } break;
    }
...
static inline int saturn_rvv_issue(uint64_t params, size_t param_size, uint64_t dst, size_t dst_size)
{
    user_addr_t param_buf;
    user_addr_t dst_buf;

    int ret = pin_user_buffer(params, param_size, &param_buf);
    if (ret)
        return ret;

    ret = pin_user_buffer(dst, dst_size, &dst_buf);
    if (ret) {
        unpin_user_buffer(&param_buf);
        return ret;
    }

    struct ggml_compute_params *kparams = (struct ggml_compute_params *)param_buf.kaddr;
    struct ggml_tensor *kdst = (struct ggml_tensor *)dst_buf.kaddr;

    match_vector_op(kparams, kdst);

    unpin_user_buffer(&param_buf);
    unpin_user_buffer(&dst_buf);
...
spinlock_t nv_queue_lock;
wait_queue_head_t nv_inst_wait_queue;
wait_queue_head_t fence_queue;
volatile int nv_queue_initialized = 0;

// for linkedlist based queue
nv_entry_t *nv_queue_dequeue(nv_queue_t *queue) {
    nv_entry_t *entry = kmalloc(sizeof(nv_entry_t), GFP_KERNEL);
    nv_node_t *node_to_remove;
    unsigned long flags;
...
nv_queue_t nv_queue_1;

void nv_queue_init(nv_queue_t *queue) {
    queue->head = NULL;
    queue->tail = NULL;
    queue->count = 0;
}
...
static int scheduler_init(void)
{
    spin_lock_init(&nv_queue_lock);
    spin_lock_init(&fence_info.gemmini_fence_lock);
    init_waitqueue_head(&nv_inst_wait_queue);
    init_waitqueue_head(&fence_queue);
    nv_queue_init(&nv_queue_1);
    
    nv_queue_initialized = 1;
    
    consumer = kthread_create(consumer_thread, NULL, "nv_queue_scheduler");
    if (IS_ERR(consumer)) 
    {
    ...
    printk(KERN_INFO "scheduler thread started.\n");
    return 0;
}
...
static int scheduler_exit(void)
{
    wake_up_all(&nv_inst_wait_queue);
    wake_up_all(&fence_queue);
    kthread_stop(consumer);
    printk(KERN_INFO "nv_queue consumer thread stopped.\n");
    consumer = NULL;
...
static int consumer_thread(void *arg)
{
    ...
    while (!kthread_should_stop())
    {
        ...
        if (nv_queue_isEmpty(&nv_queue_1)) {
...
static int consume_operation(void)
{
    nv_entry_t *op = nv_queue_dequeue(&nv_queue_1);
    ...
    if (op->mm)
        kthread_use_mm(op->mm);

    printk(KERN_INFO "current->mm=%px active_mm=%px op->mm=%px\n",
       current->mm, current->active_mm, op->mm);

    int ret = -1;
    
    if(op->cmd_type)
    {
        ret = saturn_rvv_issue(op->params, op->param_size, op->dst, op->dst_size);
    }
    else
    {
